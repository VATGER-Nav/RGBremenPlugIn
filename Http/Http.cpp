...
std::string Http::get(const std::string& url) {
    ...
    try {
        ...
        if (!success) {
            Http::throwLastError("HttpQueryInfoA");
        }
...
std::tuple<std::string, INTERNET_PORT, DWORD, std::string> Http::parseURL(std::string url) {
...
[[noreturn]] void Http::throwLastError(const std::string& fN) {
