...
namespace Calculation {
	double calculateTAS(double hdg, double gs, ::Weather::WeatherReferenceLevel lvl);
	double calculateTemperature(double alt);
	double calculateStaticPressure(double alt);
	double calculateDynamicPressure(double ps, double temp, double tas);
	double calculateSpeedOfSound(double temp);
	double calculateCAS(double alt, double hdg, double gs, ::Weather::WeatherReferenceLevel lvl);
	double calculateMach(double alt, double hdg, double gs, ::Weather::WeatherReferenceLevel lvl);
	...
	constexpr double SPEED_OF_SOUND = 340.27; // in m/s https://en.wikipedia.org/wiki/Speed_of_sound#Tables
	...
	constexpr double SPECIFIC_GAS_CONSTANT_DRY_AIR = 287.052874; // in J/kg/K https://en.wikipedia.org/wiki/Gas_constant#Specific_gas_constant
