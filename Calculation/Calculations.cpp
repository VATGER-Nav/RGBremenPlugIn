...
double Calculation::calculateTAS(double hdg, double gs, ::Weather::WeatherReferenceLevel lvl)
{
	if (gs <= 0) {
		throw std::domain_error("ground speed outside of supported range");
	}

	double c = std::cos(Calculation::deg2rad(lvl.windDirection - hdg));
	double windComponent = c * lvl.windSpeed;
	return gs + windComponent;
}

double Calculation::calculateTemperature(double alt)
{
...
double Calculation::calculateStaticPressure(double alt)
{
	double altM = (double)alt * METER_PER_FEET;
...
double Calculation::calculateDynamicPressure(double ps, double temp, double tas)
{
	if (tas <= 0) {
...
double Calculation::calculateSpeedOfSound(double temp)
{
	return std::sqrt(HEAT_CAPACITY_RATIO_AIR * SPECIFIC_GAS_CONSTANT_DRY_AIR * temp);
}

double Calculation::calculateCAS(double alt, double hdg, double gs, ::Weather::WeatherReferenceLevel lvl)
{
...
double Calculation::calculateMach(double alt, double hdg, double gs, ::Weather::WeatherReferenceLevel lvl)
{
	double tas = calculateTAS(hdg, gs, lvl);
